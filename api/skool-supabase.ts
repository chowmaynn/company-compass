...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  const url = `https://zfypovnnagzycetggazy.supabase.co/${proxyPath}${qs ? `?${qs}` : ""}`;

  const response = await fetch(url, {
    method: req.method || "GET",
    headers: {
      "apikey": process.env.SKOOL_SUPABASE_ANON_KEY!,
      "Authorization": `Bearer ${process.env.SKOOL_SUPABASE_ANON_KEY}`,
